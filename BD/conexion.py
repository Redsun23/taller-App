...
class DAO():
    ...
    def listarTrabajador(self):
        if self.conexion.is_connected():
            try:
                ...
                cursor.execute('SELECT `rut`, `nombre_completo`, `sexo`, `cargo` FROM `trabajado`;')  #Nombre tabla con Mayus.
    ...
    def registrarTrabajador(self, trabajador):
        if self.conexion.is_connected():
            try:
                cursor = self.conexion.cursor()
                sql = "INSERT INTO trabajado (rut, nombre_completo, sexo, cargo, direccion, telefono, fecha_ingreso) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}')"
